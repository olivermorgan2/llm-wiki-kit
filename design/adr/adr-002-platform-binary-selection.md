...
- Pros: fully offline / self-contained — no download at install or run, so
  ...
  resolves a single matching `bin/llm-wiki`; a checksum manifest shipped
  alongside is verified before execution (PRD §14); simplest to test on all five
  platforms in CI (criterion 21).
- Trust boundary: the per-binary checksums are generated by release CI at build
  time and shipped as a manifest embedded in the plugin payload next to the
  binaries. Because the manifest travels **inside the same payload** as the
  binaries it covers, checksum verification catches a **wrong-platform,
  mismatched, truncated, or corrupted** binary — not a malicious rewrite of the
  whole plugin/release payload, where an attacker could recompute the manifest
  to match a tampered binary. Closing that class needs a trust root **outside**
  the payload (e.g. release signing / provenance attestation), which this ADR
  **defers** (see Decision) rather than implying checksums resolve it.
...
**Trust root and residual risk.** Checksums are CI-generated and bundled with
the binaries, so verification is an **integrity** check against accidental
corruption / wrong-platform / mismatched packaging, **not** an authenticity
check against a maliciously rebuilt payload (a whole-payload attacker can
regenerate the manifest). Cryptographic **signing / provenance attestation** —
a trust root anchored outside the plugin payload — is the mechanism that would
address deliberate tampering, and it is **explicitly deferred** (candidate for
the install/packaging ADR-009 or a dedicated supply-chain ADR). The residual
"supply chain / tampered binary" risk therefore stays **open** in
`knowledge/risks.md`; this ADR narrows it (catches corruption/mismatch) but
does not retire it.
...
- Maintain: the OS+arch selection shim, the per-platform build/checksum
  ...
  risk and **narrows** — does not retire — the supply-chain risk in
  `knowledge/risks.md`; signing/provenance deferred).
- Deferred / validation implications: advances Q7 (record in `knowledge/log.md`).
  ...
  and owned by ADR-009. **Release signing / provenance attestation** (the trust
  root against malicious payload tampering) is likewise deferred — residual risk
  tracked in `knowledge/risks.md`. Criteria 2 and 21 are the acceptance hooks; both are
