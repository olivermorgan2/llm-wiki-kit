...
| Q2 | Exact Go YAML library and supported Go version for development. | Oliver | open · **assumption-locked → [ADR-001](../design/adr/adr-001-go-toolchain-and-yaml.md) proposed** (pending human acceptance) → Go 1.24.x (conservative current-stable line) + `github.com/goccy/go-yaml` (node-aware, round-trip-preserving; `yaml.v3` is archived). ADR-001 **ratifies** this lock but ships `proposed`; flip to `closed` only on human acceptance, with a [`log.md`](log.md) back-reference. Does not block scoping. |
...
| Q7 | Exact packaging mechanism for selecting the correct platform binary inside the plugin. | Oliver | open · **assumption-locked → [ADR-002](../design/adr/adr-002-platform-binary-selection.md) proposed** (pending human acceptance) → one selection mechanism ([addendum 002 A3](../design/prd-addenda/002-mvp-planning-assumptions.md)); ADR-002 advances this (ship/select/verify only — install/upgrade/uninstall asset ownership deferred to ADR-009). Flip to `closed` only on human acceptance. |
| Q8 | JSON contract versioning and compatibility policy. | Oliver | open · **assumption-locked → [ADR-003](../design/adr/adr-003-json-contract-and-exit-codes.md) proposed** (pending human acceptance) → starts at v1, no backward compat until first release ([addendum 002 A4](../design/prd-addenda/002-mvp-planning-assumptions.md)). Flip to `closed` only on human acceptance. |
