...
| Sourced claims lose provenance | Require resolvable citations; preserve existing citations. **Mechanism now decided** in [ADR-008](../design/adr/adr-008-provenance-and-citation-model.md) (accepted 2026-07-03, Codex `READY` 5/5): citations are ordinary inline links resolved by a total deterministic offline three-class predicate; preservation is enforced by a plan-time citation-loss gate routed through the ADR-003 `approval` member. Risk stays **open** — the mechanism is decided but retires only when Phase 3/4 implements and fixture-proves it (criterion 9). Content-provenance only; supply-chain signing/attestation is a **separate deferred ADR** (see the supply-chain row below). | open |
...
| Cross-platform binaries drift | Automate release builds, checksums, and platform integration tests. Bounded by [ADR-002](../design/adr/adr-002-platform-binary-selection.md) (accepted). **Implemented** Phase 2: multi-platform release bundle + checksum manifest + per-platform selfcheck smoke (#17/PR #28) and a five-platform CI matrix (#15/PR #26). Residual observation gap — the **macos-amd64** CI leg never dispatched, so that platform is covered by inference only (follow-up [#30](https://github.com/olivermorgan2/llm-wiki-kit/issues/30)); windows-amd64 full suite is red on pre-existing `internal/` permission tests (follow-up [#29](https://github.com/olivermorgan2/llm-wiki-kit/issues/29)). | open |
